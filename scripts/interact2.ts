...
async function main() {
  ...
  const amount5 = ethers.parseEther('0.002')
  ...
  await network.provider.send('hardhat_setBalance', [
   ...
   let [spender4, _amount4, No4, status4] = await multisig.getTransaction(2)
  let [spender5, _amount5, No5, status5] = await multisig.getTransaction(2)
  ...
  let [spender5A, _amount5A, No5A, status5A] = await multisig.getTransaction(2)
